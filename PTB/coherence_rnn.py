...
#inp = 3000. #400. #1300. #400
#out = 6000 #800 #2600 #800
#welchBound = ( (out-inp)/( (out-1)*inp ) )**0.5
...
def get_co_loss(weights):
  ...
  eye = linalg_ops.eye(output_size // 4)
  # i_co
  coherence_loss += math_ops.abs(math_ops.reduce_max(math_ops.abs(
    math_ops.matmul(weights[:, :output_size // 4], weights[:, :output_size // 4], transpose_a=True)) - eye))
  # f_co
  coherence_loss += math_ops.abs(math_ops.reduce_max(math_ops.abs(
    math_ops.matmul(weights[:, output_size // 4: output_size // 2], weights[:, output_size // 4: output_size // 2],
                    transpose_a=True)) - eye))
  # o_co
  coherence_loss += math_ops.abs(math_ops.reduce_max(math_ops.abs(
    math_ops.matmul(weights[:, output_size // 2: 3 * output_size // 4],
                    weights[:, output_size // 2: 3 * output_size // 4], transpose_a=True)) - eye))
  # g_co
  coherence_loss += math_ops.abs(math_ops.reduce_max(math_ops.abs(
    math_ops.matmul(weights[:, 3 * output_size // 4:], weights[:, 3 * output_size // 4:], transpose_a=True)) - eye))
  coherence_loss *= coherence * 0.25  # *(i_co+f_co+o_co+g_co)
